...
export const EventList = () => {
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 min-h-screen">
      ...
      {!!events.length && (
        <div className="grid grid-cols-1 gap-4 gap-y-12 md:grid-cols-2 lg:grid-cols-3 mt-12 md:mt-12 px-[2rem] justify-items-center">
          {events.map((event) => (
      ...
      {!events.length && !loading && (
        <div className="flex flex-1 items-center justify-center rounded-lg shadow-sm">
          <div className="flex flex-col items-center gap-1 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">There are no events available.</p>
          </div>
        </div>
      )}
