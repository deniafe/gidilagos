...
import { Event, EventAddress, EventLinks, EventOrganization, FormSchema, LGA, Period } from "@/lib/types";
...
enum STEPS {
  ...
  ADDRESS = 3,
  LINKS = 4,
  ORGANIZATION = 5, // New step for the organization tab
}


export function EventForm({data}: Props) {
  ...
  const handleSubmit = async (values: z.infer<typeof FormSchema>) => {
    try {
      ...
      const organization: EventOrganization = {
        ...
        website: values.organizationWebsite
      }
  ...
  if (step === STEPS.ORGANIZATION) {
    bodyContent = <OrganizationForm form={form} />;
  }
