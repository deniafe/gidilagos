...
// export const getOutputHTMLFromElement = (elementId: string): string => {
//   const element = document.getElementById(elementId);

//   if (element) {
//     const indentation = '  '; // Adjust the indentation as needed
//     const elementHtml = element.outerHTML;
//     const indentedHtml = elementHtml.replace(/^(.*)$/gm, indentation + '$1');

//     return indentedHtml;
//   } else {
//     alert('Failed to access element content.');
//     return '';
//   }
// };
...
const generateComponentCode = (
  ...
  const childString = generateChildString(props.children, props.text, level + 1);

  if (nodes.length === 0 && Object.keys(linkedNodes).length === 0) {
    return `${indentation}${openingTag}${childString}${closingTag}`;
  } else {
    const childComponents = nodes.map((childId) =>
      generateComponentCode(nodesMap, childId, level + 1)
    );
    ...
    return `${indentation}${openingTag}${childString}${childComponentsString}${linkedChildComponentsString}\n${indentation}${closingTag}`;
  }
};
...
const generatePropsString = (props: {
  ...
  return `${classString}${styleAttr}${idAttr}${srcAttr}`;
};
...
const generateChildString = (
  ...
  const childText = text ? text : '';
  const childNodes = Array.isArray(children) && children.length > 0
    ? children
        .map((child) => generateComponentCode({ TEMP: child }, 'TEMP', level))
        .join('')
    : '';

  return childText + childNodes;
};
